...
function getFavoris($idP){
    // Connexion à la base de données
    $cnx = new PDO("mysql:host=".HOST.";dbname=".DBNAME, DBLOGIN, DBPWD);
    // Requête SQL pour récupérer le menu avec des paramètres
    $sql = "select M.id, M.length, M.name, M.image, M.min_age from Movie as M JOIN Favoris as F ON M.id = F.id_movie WHERE F.id_profil = $idP";
    // Prépare la requête SQL
    $stmt = $cnx->prepare($sql);
    // Exécute la requête SQL
    $stmt->execute();
...
function getMoviesCategory($categ,$ageP){
    // Connexion à la base de données
    $cnx = new PDO("mysql:host=".HOST.";dbname=".DBNAME, DBLOGIN, DBPWD);
    // Requête SQL pour récupérer le menu avec des paramètres
    $sql = "select id, length, name, image, min_age from Movie WHERE id_category = $categ AND min_age <= $ageP";
    // Prépare la requête SQL
    $stmt = $cnx->prepare($sql);
    // Exécute la requête SQL
    $stmt->execute();
...
function updateProfil($nom,$image,$age,$id){
    $cnx = new PDO("mysql:host=".HOST.";dbname=".DBNAME, DBLOGIN, DBPWD); 
    $sql = "UPDATE Profils SET nom = :nom, image = :image, age = :age WHERE id = :id";
    $stmt = $cnx->prepare($sql);
    $stmt->bindParam(':nom', $nom);
    $stmt->bindParam(':image', $image);
    $stmt->bindParam(':age', $age);
...
function ajoutFavori($idP,$idF){
    $cnx = new PDO("mysql:host=".HOST.";dbname=".DBNAME, DBLOGIN, DBPWD); 
    $sql = "INSERT INTO Favoris (id_profil, id_movie) VALUES (:idP, :idF);";
    $stmt = $cnx->prepare($sql);
...
function enleveFavori($idP,$idF){
    $cnx = new PDO("mysql:host=".HOST.";dbname=".DBNAME, DBLOGIN, DBPWD); 
    $sql = "DELETE FROM Favoris WHERE id_profil = :idP AND id_movie = :idF;";
    $stmt = $cnx->prepare($sql);
    $stmt->bindParam(':idP', $idP);
    $stmt->bindParam(':idF', $idF);
    $stmt->execute();
...
function verifierFavori($idP,$idF){
    $cnx = new PDO("mysql:host=".HOST.";dbname=".DBNAME, DBLOGIN, DBPWD); 
    $sql = "SELECT * FROM Favoris WHERE id_profil = :idP AND id_movie = :idF";
    $stmt = $cnx->prepare($sql);
    $stmt->bindParam(':idP', $idP);
    $stmt->bindParam(':idF', $idF);
    $stmt->execute();
